$(function() {
    function CuraEngineLegacyViewModel(parameters) {
        ...
        self.uploadDialog = $("#settings_plugin_curalegacy_import");
        self.uploadElement = $("#settings-curalegacy-import");
        ...
        self.removeProfile = function(data) {
            ...
            OctoPrint.slicing.deleteProfileForSlicer("curalegacy", data.key, {url: data.resource()})
                .done(function() {
                    self.requestData();
                    self.slicingViewModel.requestData();
                });
        ...
        self.makeProfileDefault = function(data) {
            ...
            OctoPrint.slicing.updateProfileForSlicer("curalegacy", data.key, {default: true}, {url: data.resource()})
                .done(function() {
                    self.requestData();
                });
        };
        ...
        self.testEnginePath = function() {
            OctoPrint.util.testExecutable(self.settings.plugins.curalegacy.cura_engine())
        ...
        self.onAllBound = function() {
            self.uploadDialog.on("hidden", function(event) {
                if (event.target.id === "settings_plugin_curalegacy_import") {
                    self.clearUpload();
                }
            });
        };
        ...
        self.onSlicingData = function(data) {
            if (data && data.hasOwnProperty("curalegacy") && data.curalegacy.hasOwnProperty("profiles")) {
                self.fromResponse(data.curalegacy.profiles);
            }
        };
        ...
        self.onWizardDetails = function(response) {
            if (!response.hasOwnProperty("curalegacy") || !response.curalegacy.required) return;

            if (response.curalegacy.details.hasOwnProperty("engine")) {
                self.unconfiguredCuraEngine(!response.curalegacy.details.engine);
            }
            if (response.curalegacy.details.hasOwnProperty("profile")) {
                self.unconfiguredSlicingProfile(!response.curalegacy.details.profile);
            }
        };
