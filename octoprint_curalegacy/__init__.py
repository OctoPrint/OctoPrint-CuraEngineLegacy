...
class CuraEngineLegacyPlugin(octoprint.plugin.SlicerPlugin,
	...
	def __init__(self):
		self._logger = logging.getLogger("octoprint.plugins.curalegacy")
		self._cura_logger = logging.getLogger("octoprint.plugins.curalegacy.engine")
	...
	def initialize(self):
		...
		if old_config:
			self._logger.info("Migrating settings from cura to curalegacy...")
		...
		new_path = os.path.join(self._slicing_manager._profile_path, "curalegacy")
		if os.path.exists(old_path) and not os.path.exists(new_path):
			self._logger.info("Migrating slicing profiles from cura to curalegacy...")
			import shutil
			try:
				shutil.move(old_path, new_path)
			except Exception:
				self._logger.exception("Error while migrating slicing profiles from cura to curalegacy")

		# migrate selected default profile
		old_default = self._settings.global_get(["slicing", "defaultProfiles", "cura"])
		if old_default:
			self._logger.info("Migrating default slicing profile from cura to curalegacy...")
			self._settings.global_set(["slicing", "defaultProfiles", "curalegacy"], old_default)
	...
	def _is_profile_available(self):
		return bool(self._slicing_manager.all_profiles("curalegacy", require_configured=False))
	...
	def import_cura_profile(self):
		...
		try:
			self._slicing_manager.save_profile("curalegacy",
			                                   profile_name,
			                                   profile_dict,
	...
	def get_assets(self):
		return {
			"js": ["js/curalegacy.js"],
			"less": ["less/curalegacy.less"],
			"css": ["css/curalegacy.css"]
		}
